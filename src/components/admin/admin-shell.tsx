  ...
  return (
    <div className="min-h-screen flex bg-secondary/30">
      ...
      <aside className="hidden md:flex w-60 shrink-0 flex-col border-r border-border/60 bg-sidebar">
        ...
        <nav className="flex-1 p-2 space-y-1 overflow-y-auto nice-scroll">
          {NAV.map((item) => (
            <button
              key={item.id}
      ...
      {/* Mobile nav drawer — uses Radix Sheet for accessibility (focus trap, ARIA, Escape) */}
      <Sheet open={mobileNavOpen} onOpenChange={setMobileNavOpen}>
        <SheetContent side="left" className="w-72 p-0 flex flex-col">
          <SheetHeader className="p-4 border-b border-border/60 space-y-0">
          ...
          <nav className="flex-1 p-2 space-y-1 overflow-y-auto nice-scroll" aria-label="Admin navigation">
            {NAV.map((item) => (
              <button
                key={item.id}
          ...
          <div className="p-3 border-t border-border/60 space-y-1">
            <Button variant="outline" size="sm" className="w-full" onClick={() => { setView('storefront'); setMobileNavOpen(false) }}>
